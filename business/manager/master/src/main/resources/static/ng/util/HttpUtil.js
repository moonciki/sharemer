		...
		request: function(options) {
			...
			return $.ajax({
				...
				if (textStatus !== 'success') {
					...
					bootbox.alert('Ajax 请求异常 (see console for more details)');
					return;
				}

				if (typeof resp.code != 'undefined' && resp.code != 0) {
					var hint = '请求失败，请参考:<br/>code: ' + resp.code;
					(resp.message) && (hint += '<br/>message: ' + resp.message);
					bootbox.alert(hint);
					return;
				}
